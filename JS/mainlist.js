      ...
      if(level.progresses[0] == "none"){
        ...
        for(let i = 0; i < level.progresses.length; i++){
          ...
          if(i == 0){
            progressor.innerHTML = `
              <h5>Progresses (${level.minimumPercent}% Required)<br></h5>
              <h6>(${level.progresses[i].hertz}) ${level.progresses[i].name} (${level.progresses[i].percent}%) - <a href = "${level.progresses[i].link}" target = "_blank">${level.progresses[i].link}</h6>
            `;
          }else{
            progressor.innerHTML = `
              <h6>(${level.progresses[i].hertz}) ${level.progresses[i].name} (${level.progresses[i].percent}%) - <a href = "${level.progresses[i].link}" target = "_blank">${level.progresses[i].link}</h6>
